...
Fixes for stable car IDs:
  - NMS on detections BEFORE association (duplicate YOLO boxes were spawning new IDs)
  - iou_threshold = 0.15  (tolerates Kalman prediction uncertainty)
  - max_age = 50           (track survives short occlusions)
  - min_hits = 1           (label shows on first frame)
  - Class blocking only after >= 5 hits
"""
...
def nms(dets, iou_thresh=0.45):
    """
    NMS on (N,6) detections [x1,y1,x2,y2,conf,cls].
    Removes duplicate boxes so the tracker never sees two boxes
    for the same car that would cause a spurious ID switch.
    """
    if len(dets) == 0:
        return dets
...
class KalmanFilter:
    def __init__(self, x_init):
        ...
        self.R = np.eye(4)
        self.R[0,0]=1.; self.R[1,1]=1.; self.R[2,2]=10.; self.R[3,3]=10.
...
class KalmanBoxTracker:
    ...
    def predict(self):
        ...
        if x_pred[2,0] <= 0:
            x_pred[2,0] = 1e-3; self.kf.x[2,0] = 1e-3
        self.age += 1
    ...
    def _bbox_to_z(bbox):
        ...
        s=w*h; r=float(w)/float(h) if h>0 else 1.
    ...
    def _x_to_bbox(x):
        ...
        w=np.sqrt(max(s*r,0.)); h=(s/w) if w>0 else 0.
        return np.array([x[0]-w/2.,x[1]-h/2.,x[0]+w/2.,x[1]+h/2.]).reshape((1,4))


def associate_detections_to_trackers(detections, trackers,
                                     det_classes, trk_classes,
                                     active_trackers, iou_threshold=0.15):
    n_det=len(detections); n_trk=len(trackers)
    if n_det==0:
        return (np.empty((0,2),dtype=int),np.empty((0,),dtype=int),np.arange(n_trk,dtype=int))
    if n_trk==0:
        return (np.empty((0,2),dtype=int),np.arange(n_det,dtype=int),np.empty((0,),dtype=int))
    ...
    for d in range(n_det):
        ...
        for t in range(n_trk):
            tc=int(trk_classes[t]); trk=active_trackers[t]
            if dc>=0 and tc>=0 and dc!=tc and trk.hits>=5:
                iou_matrix[d,t]=0.

    matched_indices = (linear_assignment(-iou_matrix)
                       if iou_matrix.size>0 else np.empty((0,2),dtype=int))
    ...
    for m in matched_indices:
        if iou_matrix[m[0],m[1]] < iou_threshold:
            unmatched_dets.append(int(m[0])); unmatched_trks.append(int(m[1]))
        else:
...
class Sort:
    def __init__(self, max_age=50, min_hits=1, iou_threshold=0.15, nms_thresh=0.45):
    ...
    def reset(self):
        self.trackers=[]; self.frame_count=0; KalmanBoxTracker._count=0

    def update(self, dets=np.empty((0,6))):
        self.frame_count += 1

        # Remove duplicate detections BEFORE associating with tracks
        if len(dets) > 0:
            dets = nms(dets, self.nms_thresh)

        trks=np.zeros((len(self.trackers),4)); to_del=[]
        for t,trk_obj in enumerate(self.trackers):
            pos=trk_obj.predict()[0]; trks[t]=pos
        ...
        matched,unmatched_dets,unmatched_trks = associate_detections_to_trackers(
            dets_boxes,trks,det_classes,trk_classes,self.trackers,self.iou_threshold)
